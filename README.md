...
The official SwiftVR checkpoint is downloaded automatically on first use when `checkpoint_dir` is set to `auto` or left empty in the **SwiftVR Model Loader** node.
...
- `checkpoint_dir`: use `auto` to auto-download and load `ComfyUI/models/SwiftVR`, or enter a custom checkpoint directory.
- `device`: `cuda` or `cpu`.
...
1. Leave `checkpoint_dir` as `auto` for automatic model download, or set it to a local checkpoint folder.
