...
The official SwiftVR checkpoint is downloaded automatically on first use when `checkpoint_dir` is left empty or set to `auto` in the **SwiftVR Model Loader** node.

By default, the auto-download location is:
...
You can also manually download the model from [H-oliday/SwiftVR on Hugging Face](https://huggingface.co/H-oliday/SwiftVR):
...
You may store the checkpoint anywhere else and paste that folder path into `checkpoint_dir`.
...
- `checkpoint_dir`: leave empty / use `auto` to auto-download and load `ComfyUI/models/SwiftVR`, or enter a custom checkpoint directory.
- `device`: `cuda` or `cpu`.
...
1. Leave `checkpoint_dir` empty for automatic model download, or set it to a local checkpoint folder.
...
- The first run may take time while the model downloads from Hugging Face.
...
- **Checkpoint missing:** leave `checkpoint_dir` empty to auto-download, or verify that `reae.safetensors`, `prompt_embedding.safetensors`, and `transformer/` exist under your custom `checkpoint_dir`.
